...
所有 agent 工具的使用都需要用到 API key，这个和网页端使用不一样，不是免费的！（可能有免费渠道，但请自己甄别）你必须先给模型厂商充值，才能使用 API key 调用他们的模型。一般 deepseek 充值 10 块左右就能用很久。
...
对 Linux / macOS 用户来说，可以把上面提到的环境变量配置添加到 `~/.bashrc` 或 `~/.zshrc` 文件中，这样每次打开终端时都会自动加载这些环境变量。

对于 Windows 用户来说，可以通过系统环境变量设置来永久配置这些环境变量。具体步骤如下：

1. 打开“系统属性”窗口，可以通过以下方式之一：
   - 在开始菜单中搜索“环境变量”，然后选择“编辑系统环境变量”。
   - 右键点击“此电脑”或“我的电脑”，选择“属性”，然后点击“高级系统设置”，最后点击“环境变量”。
2. 在“环境变量”窗口中，点击“新建”按钮，添加以下环境变量：
...
skills 分全局级和项目级，一个 skill 放在全局还是项目取决于其功能范围。

全局 skill 的安装位置是：`~/.claude-code/skills`
...
一般的skills，以 [Superpower](https://github.com/obra/superpowers) 为例，会提供完整的安装说明。

如果没有提供安装说明，一般来说，下载后把 skills 文件夹放到 `~/.claude-code/skills` 目录下就可以了。
...
├── requirements.txt // 有的技能可能设计运行 python 脚本，这个文件会列出所需的 python 包
├── references/ // 参考资料，可能包含一些文档、图片等
└── scripts/ // 可能包含一些脚本文件，例如 python 脚本等
```

安装完成后，可以在 agent 中调用 `/skills` 命令获取已经加载的列表，或者用 `/a-skill` 直接使用它。
