...
目前常见的国外模型有：`claude, gemini, gpt`，有的可能还带有版本号，例如 `claude-opus-4-8`, `gpt-5.5-thinking`，`gemini-3.5-flash`（注意 anthropic 家的版本号用连字符而不是点）。\
...
目前最火的 agent 工具应该是 anthropic 的 `claude code`，简称 `cc`，它本身只原生支持 claude 系列模型，不过现在有工具能方便地接入其他模型了。\
还有类似 codex，[opencode](https://github.com/anomalyco/opencode)，[pi](https://github.com/earendil-works/pi) 等一系列工具，这里就不一一举例。\
...
- 如何安装 agent 工具 （安装 harness）
- 如何接入模型
- 如何拓展 agent 的能力（安装 skills）
...
**注意！** 永远不要运行你不知道来源的命令，部分命令可能参杂有害代码，对电脑和财产造成危害，所以一定要确认命令的来源和内容。

这行命令是直接从官网复制的，可能会随着时间变化而变化，如果你发现官网上的命令和这里不一样，请以官网上的为准。
...
**注意！** deepseek 的 API key 只在创建时可见，一定要妥善保管。它是非常敏感的信息——拿到它的人就能用你的余额调用模型，和银行卡密码一个性质，千万不要泄露给任何人，包括我！不管是弄丢了还是不小心泄露了，都请立即删除这个 key 并重新创建。
...
skills 分用户级和项目级（和前面说的配置分级一致），具体把一个 skill 安装在哪一级取决于其功能范围。

用户级 skill 的安装位置是：`~/.claude/skills`。\
...
很多人提到 claude code，会误认为只能用来写代码，但是因为其能阅读本地文件的特性，所以也可以用来处理一些日常 workflow。

下面以大家 ~~喜闻乐见~~ 的汇报PPT为例，讲解如何用 skill: [ppt-master](https://github.com/hugohe3/ppt-master) 生成PPT。

（非必需项）如果你的 PPT 刚需视觉元素，建议选择一个 **多模态** 模型接入 claude code，例如 kimi-k2.6/minimax-m3，deepseek 所有模型能上传图片的本质是通过 OCR 提取图片中的文字进行分析。

首先，新建一个文件夹作为 claude code 工作区，claude code 必须运行在工作区里。

我一般会使用 `claude --dangerously-skip-permissions` 指令打开 claude code，这串东西的意思是自动批准所有请求，直接运行。所以如果你对模型能力保持怀疑，或你不确定你的模型来源，请不要使用此功能，并务必核对每条操作指令。

然后，安装 [ppt-master](https://github.com/hugohe3/ppt-master)，官网有不同系统的安装提示，如果你不确定，直接让 claude code 为你安装即可。

![install-ppt-master](install-ppt-master.png)

然后把参考的模板和资料丢到工作文件夹中，使用类似以下的提示词。

```text
/ppt-master 生成用于 [你的使用场景] 的演示文稿，参考 @[如果你有模板，此处填模板路径] 的格式，参考资源 @[你要讲的相关文件路径]
```

一般模型可能会再问你几个相关设计问题，回答之后就会开始生成 PPT。

如果一切顺利，AI会告诉你演示文稿的路径，审阅过后可以提出修改意见（例如全局换字体，某一页的排版更换等）
